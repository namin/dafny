...
/*
    ...
    module TestModule {
        ...
        lemma TestMethod(t: tm, c: map<int, ty>)
            ensures has_type(c, t).Some? ==> has_type(c, t) == has_type(c, subst(0, tvar(0), t))
        {
        ...
        Assert.Contains("match t {", proofSketch);
        Assert.Contains("case tvar(id) => {", proofSketch);
        Assert.Contains("case tabs(x, T, body) => {", proofSketch);
        Assert.Contains("case tapp(f, arg) => {", proofSketch);
